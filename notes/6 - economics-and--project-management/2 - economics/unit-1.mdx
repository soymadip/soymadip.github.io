...
![Individual Demand](./assets/indi-demand.webp)

Individual demand refers to the quantity of a particular good or service that a single consumer
is willing and able to buy at different prices during a given period of time, asuming all other factors constant.
...
![Market Demand](./assets/market-demand.webp)
...
- $PED > 1 \rightarrow$ Demand is elastic: Consumers are very responsive to price changes.
- $PED < 1 \rightarrow$ Demand is inelastic: Consumers are not very responsive to price changes.
- $PED = 1 \rightarrow$ Unit elastic: Percentage change in demand equals percentage change in price.
...
- $Qd \rightarrow$ Quantity of Demanded
- $P \rightarrow$ Price of goods
- $I \rightarrow$ Income
- $P_r \rightarrow$ Price of related goods
- $T \rightarrow$ Tastes and Preference
- $E \rightarrow$ Expectation
- $N \rightarrow$ Number of buyers
...
$TR = Price(Q) \times quantity(Q)$

![relation-between-price-and-tr](./assets/relation.jpeg)

**If**:

- $E_p > 1 \rightarrow$ TR increase and price decrease.
- $E_p < 1 \rightarrow$ TR decrease and price increase.
- $E_p = 1 \rightarrow$ TR is unchanged.
...
- $G_t =$ government policies (Taxes & subsidies)

:::note
Above are <u>key determinants of supply function</u> too
:::
...
- $MS =$ Market Supply
- $\sum S_x =$ Sum of individul supplies of all sellers
