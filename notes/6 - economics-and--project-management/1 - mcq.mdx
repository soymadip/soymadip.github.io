...
- **Explanation:** Average Fixed Cost ($AFC = Total Fixed Cost / Quantity$) continuously falls as output increases because a constant total cost is being divided by a larger number of units. However, it never reaches zero, and the area under the curve ($AFC \times Q = TFC$) remains constant, giving it the mathematical shape of a rectangular hyperbola.
...
- **Explanation:** Marginal Product of Labor ($MP_L$) is defined as the additional output produced by employing one extra unit of labor while keeping all other production factors constant ($MP_L = \Delta TP / \Delta L$).
...
- **Answer:** (a) falls
- **Explanation:** Average Fixed Cost is calculated as Total Fixed Cost divided by output ($AFC = TFC / Q$). Because TFC remains constant in the short run, dividing it by an increasing quantity ($Q$) means AFC will continuously decline as output expands.
