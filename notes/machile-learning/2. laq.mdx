...
| Learns a mapping from inputs to known outputs and predicts future data.                  | Discovers hidden patterns and structures in data.                           |
| Receives feedback by comparing predictions with actual labels.                           | No feedback mechanism because there are no known labels.                    |
...
| Generally less complex and easier to train.                                              | Generally more complex and computationally intensive.                       |
| Used for Classification and Regression.                                                  | Used for Clustering, Dimensionality Reduction, and Association Rule Mining. |
| Requires manually labeled data before training.                                          | Requires little or no manual labeling.                                      |
...
## 6. What do you mean by clustering? Diff types of clustering

Clustering is an unsupervised learning technique in Machine Learning that groups similar data objects into the same cluster and places dissimilar objects into different clusters.
...
Assumes that data is generated from a mixture of probability distributions and fits a model to the data.
...
| Performance is measured using MAE, MSE, RMSE, and R² Score.             | Performance is measured using Accuracy, Precision, Recall, and F1-Score. |

---

## 10. Types of clustering. with example

Clustering is the unsupervised learning technique that groups similar data objects into clusters
so that objects within the same cluster are more similar to each other than those in different clusters.

### A. Partitioning Clustering

Divides data into a fixed number of clusters.

**Example**: K-Means Clustering groups customers into different segments based on purchasing behavior.

### B. Hierarchical Clustering

Creates a hierarchy of clusters represented as a tree structure (dendrogram).

**Example**: Grouping animals based on characteristics such as habitat, diet, and size.

### C. Density-Based Clustering

Forms clusters in dense regions of data and identifies outliers in sparse regions.

**Example**: DBSCAN is used to detect geographical hotspots from GPS location data.

### D. Grid-Based Clustering

Divides the data space into a grid of cells and performs clustering on the grids.

**Example**: Used in geographic information systems (GIS) for analyzing spatial data.

### E. Model-Based Clustering

Assumes that data is generated from a statistical model and clusters data accordingly.

**Example**: Gaussian Mixture Models (GMM) used for image segmentation and pattern recognition.
...
in which an agent learns by interacting with an environment and receiving rewards or penalties for its actions.
