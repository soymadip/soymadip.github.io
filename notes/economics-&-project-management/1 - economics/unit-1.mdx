...
$TR = \text{Price(Q)} \times \text{quantity(Q)}$

![relation-between-price-and-tr](./assets/relation.jpeg)

**If**:

- $E_p > 1 \rightarrow \text{TR increase and price decrease}.$
- $E_p < 1 \rightarrow \text{TR decrease and price increase}.$
- $E_p = 1 \rightarrow \text{TR is unchanged}.$

---

## 11. What are the causes of scarcity of resources?

**scarcity** is the economic condition where limited resources are insufficient to satisfy unlimited human wants.
...
- $\sum S_x = \text{Sum of individul supplies of all sellers}$
