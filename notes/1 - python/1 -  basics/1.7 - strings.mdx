...
source: "/code/python/1-primitive-data-types/strings.py"
---
...
Strings are **immutable**—we cannot change a character in place. If we're coming from C, we might be used to changing an array in place; in Python, we have to create a _new_ string instead.
...
Slicing uses the `[start:stop:step]` format. Remember that the "stop" number is exclusive (it stops _before_ that index).
