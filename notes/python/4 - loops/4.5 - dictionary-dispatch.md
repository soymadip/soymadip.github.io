...
When you have a long list of `if/elif` or `match-case` blocks that simply return different values based on a single key, you can use a dictionary as a **lookup table** (or dispatch table). This maps keys directly to values or functions, making your code cleaner and easier to maintain.
...
If you need to add more rules, you just update the `rules` dictionary. The actual logic inside the `for` loop doesn't have to change at all, separating your **data** from your **logic**.

---

_Source file: [37.py](/code/python/4-loops/37.py)_
