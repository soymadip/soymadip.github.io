...
Python lets you return multiple values separated by commas. They are automatically packed into a **tuple**, so the caller gets one object back.
...
Unpacking requires the exact number of variables as values in the tuple. Use `_` to discard values you don't need.
...
_Source file: [43.multiple-returns.py](/code/python/5-functions/43.multiple-returns.py)_
