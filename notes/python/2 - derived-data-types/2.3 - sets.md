...
Sets are collections of unique elements. They are the go-to tool in Python when you need to remove duplicates or perform fast membership checks (`O(1)` time). Use `set()` to create an empty set, as `{}` creates an empty dictionary.
...
Standard sets are mutable. If you need a set that cannot be changed—for example, to use it as a key in a dictionary—use `frozenset`.
...
_Source file: [19 - sets.py](/code/python/2-derived-data-types/19-sets.py)_
