...
Sets are collections of unique elements. They are the go-to tool in Python when we need to remove duplicates or perform fast membership checks (`O(1)` time). Use `set()` to create an empty set, as `{}` creates an empty dictionary.
...
<SourcePreview path="/code/python/2-derived-data-types/19-sets.py" label="19 - sets.py" />
