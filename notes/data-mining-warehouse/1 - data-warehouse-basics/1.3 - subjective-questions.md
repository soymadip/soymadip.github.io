...
1.  **Enterprise Data Warehouse (EDW):** A centralized warehouse for the entire organization that stores data from all departments. Used for overall business analysis. *Example:* A company stores sales, HR, and finance data in one system.
2.  **Data Mart:** A small part of a data warehouse focused on one specific department. It is easier and faster to use. *Example:* Only sales department data stored for analysis.
3.  **Virtual Warehouse:** A view of data from operational databases (not physically stored separately). Quick to create but depends on original data sources. *Example:* Data shown directly from existing databases without full physical storage.
...
3.  **Better Analysis:** Makes it easier to find patterns and trends.
4.  **Efficient Querying:** Reduces complexity and speeds up data analysis.
...
**Answer:**
- **Fact:** Numerical data used for analysis. It usually represents measurements or values. *Example:* Sales amount, profit, quantity sold.
- **Dimension:** Descriptive data that gives context to facts. It answers who, what, where, and when. *Example:* Customer name, product, city, date.

##### 8. What is a Star Schema?
**Answer:**
A star schema is a way of organizing data in a data warehouse where one central **fact table** is connected to multiple **dimension tables**, forming a shape like a star.
...
**Answer:**
A galaxy schema is a data warehouse design where multiple fact tables share common dimension tables. It looks like a collection of star schemas connected together.
**When is it used?**
